...
// local
#include "RawBankToFilteredSTClusterAlg.h"
...
//-----------------------------------------------------------------------------
// Implementation file for class : RawBankToFilteredSTClusterAlg
//
// 2015-05-06 : Elena Graverini
//-----------------------------------------------------------------------------

DECLARE_ALGORITHM_FACTORY( RawBankToFilteredSTClusterAlg )

RawBankToFilteredSTClusterAlg::RawBankToFilteredSTClusterAlg( const std::string& name,
...
STDecodingBaseAlg (name , pSvcLocator){
 ...
 // Standard constructor, initializes variables
  ...
  tt_names = new LHCb::TTNames();
}

RawBankToFilteredSTClusterAlg::~RawBankToFilteredSTClusterAlg() {
  // Destructor
}

StatusCode RawBankToFilteredSTClusterAlg::initialize() {
...
StatusCode RawBankToFilteredSTClusterAlg::execute() {
...
StatusCode RawBankToFilteredSTClusterAlg::decodeBanks(RawEvent* rawEvt, 
...
void RawBankToFilteredSTClusterAlg::createCluster( const STClusterWord& aWord,
  ...
  STChannelID thisChannelID = newCluster->channelID();
  //STChannelID thisChannelID = STChannelID((const STChannelID &) newCluster->channelID());
  unsigned int chID = thisChannelID.channelID();
  //STChannelID *thisConstId = &thisChannelID;
  //tt_names = LHCb::TTNames();
  ...
  if (!( (chID >= min_TTaULayerR2Module2T && chID <= max_TTaULayerR2Module2T) ||
    ...
    if( UNLIKELY( msgLevel(MSG::DEBUG) ) ){
      std::string thisSector = tt_names->UniqueSectorToString((const STChannelID &)thisChannelID);
...
double RawBankToFilteredSTClusterAlg::mean(const std::vector<SiADCWord>& adcValues) const
{
...
StatusCode RawBankToFilteredSTClusterAlg::finalize() {
